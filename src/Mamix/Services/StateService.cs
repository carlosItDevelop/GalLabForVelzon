...
public class AppState
{
    ...
    public string PageStyles { get; set; } = "classic";                 // regular, classic, modern
...
public class StateService
{
    ...
    public async Task colorthemeFn(string val, bool stateClick)
    {
        ...
        if (stateClick)
        {
            if (val == "dark")
            {
                await _jsRuntime.InvokeVoidAsync("interop.addAttributeToHtml", "data-header-styles", "transparent");
    ...
    public async Task retrieveFromLocalStorage()
    {
        ...
        string mamixmenuposition = _currentState.MenuPosition;
        await menuPositionFn(mamixmenuposition);
        string mamixheaderposition = _currentState.HeaderPosition;
        await headerPositionFn(mamixheaderposition);
        string mamixcolortheme = _currentState.ColorTheme;
        await colorthemeFn(mamixcolortheme, false);
        string mamixbgimg = _currentState.BackgroundImage;
        if (!string.IsNullOrEmpty(mamixbgimg))
        {
            await backgroundImageFn(mamixbgimg);
        }
        string mamixbgcolor = _currentState.ThemeBackground;
        string mamixbgcolor1 = _currentState.ThemeBackground1;
        if (!string.IsNullOrEmpty(mamixbgcolor))
        {
            await themeBackgroundFn(mamixbgcolor, mamixbgcolor1,false);
            _currentState.ColorTheme = "dark";
            // _currentState.MenuColor = "dark";
            // _currentState.HeaderColor = "dark";
        }
        string mamixMenu = _currentState.MenuColor;
        await menuColorFn(mamixMenu);
        string mamixHeader = _currentState.HeaderColor;
        await headerColorFn(mamixHeader);
        string mamixmenuStyles = _currentState.MenuStyles;
        string mamixverticalstyles = _currentState.LayoutStyles;
        if (string.IsNullOrEmpty(mamixverticalstyles))
        {
            await menuStylesFn(mamixmenuStyles);
        }
        else
        {
            await layoutStylesFn(mamixverticalstyles);
        }
        string mamixprimaryRGB = _currentState.ThemePrimary;
        await themePrimaryFn(mamixprimaryRGB);
    }
    public async Task retrieveFromLandingLocalStorage()
    {
        ...
        string direction = await _jsRuntime.InvokeAsync<string>("interop.getLocalStorageItem", "mamixdirection") ?? _currentState.Direction;
        await directionFn(direction);
        string mamixcolortheme = await _jsRuntime.InvokeAsync<string>("interop.getLocalStorageItem", "mamixcolortheme") ?? _currentState.ColorTheme;
        await colorthemeFn(mamixcolortheme, false);
        string mamixprimaryRGB = await _jsRuntime.InvokeAsync<string>("interop.getLocalStorageItem", "mamixprimaryRGB") ?? _currentState.ThemePrimary;
        await themePrimaryFn(mamixprimaryRGB);
    }
