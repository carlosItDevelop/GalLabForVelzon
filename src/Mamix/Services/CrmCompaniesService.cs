...
{
    private List<CrmCompany> CrmCompaniesLists = new List<CrmCompany>()
    {
        new CrmCompany { 
            CompaniesImg = "../assets/images/company-logos/1.png", 
            CompaniesName = "Spruko Technologies", 
            Email="spruko2981@gmail.com", 
    ...
    public List<CrmCompany> GetCrmCompany()
    {
        return CrmCompaniesLists;
    }

    public async Task DeleteCrmCompany(string? DealsId)
    {
        var CrmCompanyToDelete = CrmCompaniesLists.FirstOrDefault(i => i.DealsId == DealsId);
        if (CrmCompanyToDelete != null)
        {
            CrmCompaniesLists.Remove(CrmCompanyToDelete);
